...
#let detailed-sign-field(title, name, position, year) = {
  assert(
    type(name) == str,
    message: "Некорректный тип поля name в detailed-sign-field",
  )
  assert(
    type(position) == str,
    message: "Некорректный тип поля position в detailed-sign-field",
  )
  assert(
    type(year) in (int, type(none)),
    message: "Некорректный тип поля year в detailed-sign-field",
  )
  let year-cell = []
  let line-end = 7
  if year != none {
...
#let sign-field(name, position, part: none, details: "подпись и расшифровка подписи") = {
  let part-cell = []
  if part != none {
  ...
  table(
    ...
    [#position], [], [«__» ________ 20__ г.], [], table.cell(align: bottom)[#unbreak-name(name)],
    table.hline(start: 0, end: 5),
